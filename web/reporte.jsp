<%-- 
    Document   : reporte
    Created on : 14 jun. 2026, 11:15:29
...
<%
    ...
    Double totalSemana = (Double) request.getAttribute("totalSemana");
...
<head>
    ...
    <title>Bodega Vargas | <%= titulo %></title>
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="reporte?action=semanal" class="nav-link <%= titulo.equals("Reporte Semanal") ? "active" : "" %>">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Semanal</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="reporte?action=mensual" class="nav-link <%= titulo.equals("Reporte Mensual") ? "active" : "" %>">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Mensual</p>
                </a>
              </li>
      ...
      <div class="container-fluid">
        ...
        <% if (totalSemana != null) { %>
          <div class="alert alert-info">
            <h5>Total de la Semana: S/ <%= String.format("%,.2f", totalSemana) %></h5>
          </div>
        <% } %>
        
        <% if (totalMes != null) { %>
          <div class="alert alert-info">
            <h5>Total del Mes: S/ <%= String.format("%,.2f", totalMes) %></h5>
          </div>
        <% } %>

        <div class="card">
          <div class="card-header">
            <h3 class="card-title">Detalle de Pedidos</h3>
          </div>
